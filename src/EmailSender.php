...
class Email
{
    ...
    public function __construct($server, $port = 25, $connectionTimeout = 30, $responseTimeout = 8, $hostname = null)
    {
        ...
        $this->logs['CREATE_OBJECT'] = $this->server . ":". $this->port . " Host:" . $this->hostname;
    }
    ...
    public function send()
    {
        ...
        #echo "open\n";
        $this->logs['CONNECTION'] = $this->getResponse(). " Server=>".$this->getServer().":".$this->port;
        ...
        if (!empty($this->username)){
            ...
            $this->logs['USERNAME'] = $this->sendCommand(base64_encode($this->username))." User=".$this->username;
            $this->logs['PASSWORD'] = $this->sendCommand(base64_encode($this->password))." Pass=".$this->password;
        }
        $this->logs['MAIL_FROM'] = $this->sendCommand('MAIL FROM: <' . $this->from[0] . '>');
        $rc=substr($this->logs['MAIL_FROM'],0,3);
        #echo "+++".$this->from[0]."=>[".$rc."] ".$this->logs['MAIL_FROM'];
        if ($rc == '501' or $rc == '552') {return false;}
